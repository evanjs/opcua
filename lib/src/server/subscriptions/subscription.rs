...
impl Subscription {
        ...
        match self.state {
            ...
            SubscriptionState::KeepAlive => {
                        ...
                        || (self.publishing_enabled && !p.notifications_available))
                {
                    // State #16
                    self.start_publishing_timer();
                    let old_value = self.keep_alive_counter;
                    ...
                    debug!(
                        keep_alive_counter_old_value = old_value,
                        keep_alive_counter = self.keep_alive_counter,
                        "Keep-alive counter decremented",
                    );
    ...
    // pub fn check_publishing_timer(&mut self, now: &DateTimeUtc) {
    //     if !self.publishing_timer_expired {
    //         let elapsed = now.signed_duration_since(self.last_publishing_time).to_std().unwrap();
    //         if elapsed >= Duration::from_millis(self.publishing_interval as u64) {
    //             self.publishing_timer_expired = true;
    //         }
    //     }
    // }
